...
export default async function AdminAttendancePage({
    ...
    const employeeIds =
        teamMembers?.filter((m) => m.role === "Employee").map((m) => m.id) || [];
    ...
    // Fetch ALL records for calendar and today's stats
    ...
    const presentToday = todayRecords.filter((r) => {
        ...
        return r.status === "Present" || r.status === "Half Day";
    }).length;
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    ...
                    <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest mt-1">
                        Monitor team presence and location data
                    </p>
                </div>
            </header>
                                ...
                                <TableBody>
                                    {teamMembers
                                        ...
                                        .map((member) => (
                                            <TableRow
                                                key={member.id}
                                                className="border-border group h-16 hover:bg-red-50/10"
                                            >
                                                <TableCell className="px-6">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="font-extrabold text-[12px] text-foreground">
                                                            {member.name}
                                                        </span>
                                                    </div>
                                                </TableCell>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest">
                                Live check-in and location stream
                            </p>
                        </div>
                    </div>
                        ...
                        <Table>
                            <TableHeader className="bg-muted/30">
                                <TableRow className="border-border h-12">
                                    <TableHead className="px-6 font-bold text-[10px] text-muted-foreground uppercase tracking-widest">
                                        Team Member
                                    </TableHead>
                                    <TableHead className="font-bold text-[10px] text-muted-foreground uppercase tracking-widest">
                                        Date
                                    </TableHead>
                                    <TableHead className="font-bold text-[10px] text-muted-foreground uppercase tracking-widest">
                                        Status
                                    ...
                                    <TableHead className="text-right px-6 font-bold text-[10px] text-muted-foreground uppercase tracking-widest">
                                        Location Insights
                                    </TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {history.length > 0 ? (
                                    history.map((entry) => (
                                        <TableRow
                                            key={entry.id}
                                            className="border-border group h-20 hover:bg-muted/30 transition-all duration-300"
                                        >
                                            <TableCell className="px-6">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="font-bold text-[14px] text-foreground truncate leading-none">
                                                            {entry.user?.name}
                                                        </p>
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-2 text-[11px] font-bold text-foreground tabular-nums">
                                                    ...
                                                    {formatISTTime(entry.check_in)}
                                                    <span className="text-muted-foreground/20">/</span>
                                                    {formatISTTime(entry.check_out)}
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                ...
                                                <div className="flex flex-col items-end gap-1.5">
                                                    {entry.check_in_location && (
                                                        <div className="flex items-center gap-2 text-[9px] font-bold text-muted-foreground group-hover:text-primary transition-colors">
                                                            ...
                                                            {entry.check_in_location.latitude.toFixed(4)},{" "}
                                                            {entry.check_in_location.longitude.toFixed(4)}
                                                        </div>
                                                    )}
                                                    {entry.check_out_location && (
                                                        <div className="flex items-center gap-2 text-[9px] font-bold text-muted-foreground group-hover:text-blue-500 transition-colors">
                                                            ...
                                                            {entry.check_out_location.latitude.toFixed(4)},{" "}
                                                            {entry.check_out_location.longitude.toFixed(4)}
                                                        </div>
                                                    )}
                                                </div>
                                            </TableCell>
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={6} className="py-20 text-center">
                                            <p className="text-muted-foreground/30 text-[11px] font-bold uppercase tracking-[0.2em]">
                ...
                <div className="grid grid-cols-1 gap-4 md:hidden">
                    {history.length > 0 ? (
                        history.map((entry) => (
                            <Card
                                key={entry.id}
                                className="p-5 border-border/50 shadow-sm bg-white space-y-4"
                            >
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-bold text-[14px] text-foreground leading-none">
                                                {entry.user?.name}
                                            </p>
                                            <p className="text-[10px] font-bold text-muted-foreground uppercase mt-1.5">
                                                {new Intl.DateTimeFormat("en-IN", {
                                                    month: "short",
                                                    day: "2-digit",
                                                    timeZone: "Asia/Kolkata",
