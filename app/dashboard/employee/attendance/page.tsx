    ...
    return (
        <div className="space-y-6 mx-auto pb-20 lg:pb-0">
            <header className="flex justify-between items-center">
                <div>
                    <h2 className="text-xl font-bold tracking-tight text-foreground">Attendance & Sessions</h2>
                    <p className="text-muted-foreground text-[10px] font-bold uppercase tracking-widest mt-1">Track your work presence and history</p>
                </div>
            </header>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
                    ...
                    <Card className="border border-border/50 shadow-sm rounded-xl bg-primary text-white overflow-hidden relative group h-fit">
                        ...
                        <CardHeader className="p-6 pb-2 relative">
                            <div className="flex items-center gap-2 mb-1">
                                <span className="text-[10px] font-extrabold text-white/50 uppercase tracking-widest leading-none">ACTIVE SESSION</span>
                            </div>
                            <CardTitle className="text-xl font-bold flex items-center gap-2">
                                {format(new Date(), 'EEEE')}
                                <span className="opacity-40 text-[13px]">{format(new Date(), 'MMM do')}</span>
                            </CardTitle>
                        </CardHeader>
                            ...
                            {!todayAttendance ? (
                                ...
                                <div className="text-center space-y-6 flex flex-col items-center w-full">
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <p className="text-white/50 font-bold text-[11px] uppercase tracking-widest">Have a great rest! 👋</p>
                                    </div>
                            ...
                            {totalPages > 1 && (
                                <div className="flex items-center gap-1">
                                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                                        <Link key={p} href={`/dashboard/employee/attendance?page=${p}`}>
                                            <Button
                            ...
                            <Table>
                                <TableBody>
                                    {(history && history.length > 0) ? history.map((entry) => (
                                        <TableRow key={entry.id} className="border-border h-16 hover:bg-muted/30 transition-all duration-300">
                                            <TableCell className="px-6">
                                                <p className="font-bold text-[13px] text-foreground leading-none">{format(new Date(entry.date), 'MMM dd')}</p>
                ...
                {/* Right: Calendar (spanning 2 columns on large screens) */}
